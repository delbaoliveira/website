...
In the first part of the **building a developer blog with Next.js** series, we will setup and configure one of the key components _(or content layer)_ of our Next.js + MDX blogging system. [Contentlayer](https://contentlayer.dev) is a tool that helps deal with the chaos of freeform MDX content by transforming it into structured type-safe data and significantly reducing the boilerplate and external tools required to effectively integrate MDX content with the rest of your app. Folks have been describing it as the _"prisma for content"_.

I have been using and enjoying Contentlayer for a while now and it finally went into beta last week. Judging by the initial reaction, it seems I am not the only one who thinks it is a good idea:
...
Contentlayer transforms your posts markdown source file (line 10) into structured data and stores the result as a static `json` file (line 5). In addition, Contentlayer generates type definitions (line 7) and convience helper functions for accessing post data (line 6).

The processed `json` file includes the raw markdown content, the processed MDX code and generated fields such as the files name. It will also include custom fields we will add later.
...
Provide `getStaticPaths(){:.fn}` a list of posts to pre-render by mapping over `allPosts{:.objKey}` and transforming each posts filename into a slug:
...
> 🐣  
> Next.js will pre-render each post by looping through the list of posts you provided to `getStaticPaths(){:.fn}`, fetching the processed data from Contentlayer and passing it down as props to your page component via `getStaticProps(){:.fn}`, rendering the `SinglePostPage(){:.fn}` page component and saving the result as static files ready for deployment.

### Computed fields

In addition to Contentlayers automatically generated fields, you can add your own fields to a document type. The first kind of field are **computed fields**. Computed fields are useful for adding a new field to a document that uses its current contents to calculate something new. For instance you could use a posts contents to calculate an estimated read time.

You may have noticed that the `.replace(/\.mdx$/, ""){:js}` logic is currently being run in both `getStaticPaths{:.fn}` and `getStaticProps{:.fn}`. You could move this logic into a single computed field:
...
This covers the majority of setup and configuration. As you've hopefully realised, Contentlayer greatly simplifies the process of implementing MDX.
