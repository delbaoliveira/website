---
title: "Integrate MDX with a single tool and less boilerplate. Manage content with type-safe data and helper functions."
description: "Effortlessly integrate MDX and manage content effectively. Validate and transform markdown posts into type-safe data your app can import."
...
Stitch Next.js and Contentlayer together using the `next-contentlayer` plugin to hook into the `next dev` and `next build` processes. This automatically runs the Contentlayer CLI while you're developing locally or building your app for deployment:
...
Saving this post file while `next dev` is running will incrementally generate type definitions, helper functions and transformed static files for each document type you have defined in: `makeSource({ documentTypes: [...] }){:js}`
    ...
    items={[
      {
        ...
        items: [
            ...
            items: [
              {
                ...
                items: [
                  {
                    name: "posts__hello-world.mdx.json",
                    status: "highlighted",
                  },
                ],
              },
          ...
          { name: "index.js", status: "faded" },
          { name: "types.d.ts", status: "faded" },
        ],
      },
      {
        name: "content",
        items: [
          {
            name: "posts",
            items: [{ name: "hello-world.mdx", status: "highlighted" }],
          },
        ],
      },
      {
...
Contentlayer transforms your `hello-world.mdx` (2) markdown source file into structured data and stores the result as a static `posts__hello-world.mdx.json` (1) file. In addition, Contentlayer generates type definitions and convenience helper functions for accessing post data.
...
```json title=".contentlayer/generated/Post/posts__hello-world.mdx.json"
{
  "type": "Post",
...
~~I'd also recommend adding the `.contentlayer` folder to your `.gitignore` to reduce the git commit and diff noise from generated files.~~ **Update:** This is now handled automatically when you first generate a document.
